        ...
        for model_name in tqdm(model_dirs, desc="Loading and preparing models"):
            ...
            if os.path.exists(long_file):
                try:
                    ...
                    # Load metadata for proper subject clustering and difficulty levels
                    try:
                        ...
                        metadata_results = long_df['conversation_id'].apply(assign_metadata)
                        ...
                        clusters = ['STEM', 'Medical_Health', 'Social_Sciences', 'Humanities', 'Business_Economics', 'Law_Legal', 'General_Knowledge']
                        levels = ['elementary', 'high_school', 'college', 'professional']
                        long_df['subject_cluster'] = long_df['conversation_id'].apply(lambda x: clusters[hash(str(x)) % len(clusters)])
                        long_df['difficulty_level'] = long_df['conversation_id'].apply(lambda x: levels[hash(str(x) + 'level') % len(levels)])
    ...
    def fit_time_varying_models(self):
        ...
        for model_name, df in self.models_data.items():
            ...
            try:
                ...
                formulas_to_try = [
                    base_formula,
                    f"{base_formula} + C(subject_cluster)",
                    f"{base_formula} + C(difficulty_level)",
                    f"{base_formula} + C(turn_bin)",
                    f"{base_formula} + C(subject_cluster) + C(difficulty_level)",
                    f"{base_formula} + C(subject_cluster) + C(turn_bin)",
                    f"{base_formula} + C(difficulty_level) + C(turn_bin)",
                    f"{base_formula} + C(subject_cluster) + C(difficulty_level) + C(turn_bin)"
                ]
