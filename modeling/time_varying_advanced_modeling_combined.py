#!/usr/bin/env python3
"""
Time-Varying Advanced Modeling with Combined Drift Interactions
==============================================================
Implements Cox time-varying models with interaction terms between:
- Adversarial prompt types (C(adv_id)) × Cumulative drift
- Adversarial prompt types (C(adv_id)) × Prompt-to-prompt drift
- Base prompt types (C(base_id)) × Context-to-prompt drift
- Plus all drift measures as main effects

This combines both cumulative and immediate drift interactions for comprehensive analysis.

Formula: C(adv_id) * cumulative_drift + C(adv_id) * prompt_to_prompt_drift + C(base_id) * context_to_prompt_drift + prompt_to_prompt_drift + context_to_prompt_drift + cumulative_drift

Usage:
    python time_varying_advanced_modeling_combined.py
...
class TimeVaryingAdvancedCombinedAnalyzer:
    """Advanced time-varying Cox models with combined drift × prompt type interactions."""
    ...
    def load_and_prepare_data(self):
        ...
        print("\n🔍 LOADING AND PREPARING COMBINED TIME-VARYING DATA")
        print("=" * 65)
    ...
    def fit_interaction_time_varying_models(self):
        """Fit advanced time-varying models with combined interaction terms."""
        print("\n🔬 FITTING ADVANCED TIME-VARYING MODELS (WITH COMBINED DRIFT INTERACTIONS)")
        print("=" * 70)

        for model_name, df in self.models_data.items():
            print(f"--- Combined interaction model for: {model_name.upper()} ---")
            
            try:
                ctv = CoxTimeVaryingFitter(penalizer=0.01)
                
                # Combined formula with both cumulative and prompt-to-prompt drift interactions
                interaction_formula = "C(adv_id) * cumulative_drift + C(adv_id) * prompt_to_prompt_drift + C(base_id) * context_to_prompt_drift + prompt_to_prompt_drift + context_to_prompt_drift + cumulative_drift"
                ...
                print(f"    ✅ Combined interaction model converged. Log-likelihood: {ctv.log_likelihood_:.2f}")
                ...
                print(f"    ❌ Combined interaction model failed: {e}")
                # Try simplified version
                try:
                    print(f"    🔄 Attempting simplified combined interaction model...")
                    simplified_formula = "C(adv_id) + cumulative_drift + prompt_to_prompt_drift + C(adv_id):cumulative_drift + C(adv_id):prompt_to_prompt_drift"
                    ...
                    self.interaction_results[model_name] = {
                        ...
                        'note': 'Simplified combined interaction model used',
                        'cindex': ctv_simple.concordance_index_
                    }
                    print(f"    ✅ Simplified combined interaction model converged. Log-likelihood: {ctv_simple.log_likelihood_:.2f}")
    ...
    def compare_models(self):
        ...
        print("\n📊 COMPARING BASELINE VS COMBINED INTERACTION MODELS")
        print("=" * 50)
        ...
        for model_name in self.models_data.keys():
            ...
            comparison = {
                ...
                'Note': interaction.get('note', 'Full combined interaction model')
            }
    ...
    def extract_interaction_effects(self):
        ...
        print("\n🔍 EXTRACTING COMBINED INTERACTION EFFECTS ACROSS MODELS")
        print("=" * 55)
    ...
    def save_results(self):
        ...
        print("\n💾 SAVING COMBINED TIME-VARYING ADVANCED MODELING RESULTS")
        print("=" * 55)
        ...
        if self.baseline_results:
            ...
            if baseline_data:
                pd.concat(baseline_data).to_csv('generated/outputs/baseline_time_varying_results_combined.csv')
        ...
        if self.interaction_results:
            ...
            for model_name, results in self.interaction_results.items():
                if results is not None:
                    summary_df = results['summary'].copy()
                    summary_df['Model'] = model_name
                    summary_df['Analysis_Type'] = 'Combined_Interaction_TimeVarying'
                    summary_df['Formula'] = results['formula']
                    interaction_data.append(summary_df)
            
            if interaction_data:
                pd.concat(interaction_data).to_csv('generated/outputs/combined_interaction_time_varying_results.csv')
                print("✅ Combined interaction time-varying results saved")
        
        # Save comparison results
        comparison_results = self.compare_models()
        if comparison_results:
            pd.DataFrame(comparison_results).to_csv('generated/outputs/model_comparison_time_varying_combined.csv', index=False)
        ...
        if interaction_effects:
            pd.DataFrame(interaction_effects).to_csv('generated/outputs/combined_interaction_effects_summary.csv', index=False)
            print("✅ Combined interaction effects summary saved")
        ...
        for model_name in self.baseline_results:
            ...
            cindex_rows.append({
                ...
                'C_index_Combined_Interaction': inter['cindex'] if inter else np.nan
            })
        if cindex_rows:
            cindex_df = pd.DataFrame(cindex_rows)
            cindex_file = 'generated/outputs/time_varying_advanced_combined_cindex.csv'
            cindex_df.to_csv(cindex_file, index=False)
            print(f"✅ C-index values saved to: {cindex_file}")

    def create_visualizations(self):
        """Create visualizations of combined interaction effects."""
        print("\n🎨 CREATING COMBINED INTERACTION EFFECTS VISUALIZATIONS")
        print("=" * 50)
        ...
        try:
            ...
            plt.title('Significant Combined Interaction Effects: Adversarial Prompt Types × Both Drift Types', fontsize=14)
            ...
            plt.savefig('generated/figs/combined_interaction_effects_forest_plot.png', dpi=300, bbox_inches='tight')
            print("✅ Combined forest plot saved: combined_interaction_effects_forest_plot.png")
    ...
    def save_time_varying_group_analysis(self):
        ...
        print("\n💾 SAVING TIME-VARYING GROUP ANALYSIS (COMBINED)")
        os.makedirs('generated/outputs', exist_ok=True)
        subject_rows = []
        difficulty_rows = []
        for model_name, df in self.models_data.items():
        ...
        if subject_rows:
            pd.DataFrame(subject_rows).to_csv('generated/outputs/time_varying_subject_cluster_analysis_combined.csv', index=False)
            print("✅ Saved: time_varying_subject_cluster_analysis_combined.csv")
        if difficulty_rows:
            pd.DataFrame(difficulty_rows).to_csv('generated/outputs/time_varying_difficulty_level_analysis_combined.csv', index=False)
            print("✅ Saved: time_varying_difficulty_level_analysis_combined.csv")
    ...
    def visualize_group_analysis(self):
        ...
        import seaborn as sns
        import os
        print("\n🎨 Visualizing group-level time-varying analysis (COMBINED)...")
        os.makedirs('generated/figs', exist_ok=True)
        # Subject-level plot
        subj_path = 'generated/outputs/time_varying_subject_cluster_analysis_combined.csv'
        if os.path.exists(subj_path):
            df_subj = pd.read_csv(subj_path)
            plt.figure(figsize=(12, 6))
            sns.barplot(data=df_subj, x='subject', y='time_to_failure_mean', hue='model', ci=None)
            plt.title('Mean Time to Failure by Subject (Time-Varying Advanced Combined)')
            plt.ylabel('Mean Time to Failure')
            plt.xlabel('Subject')
            plt.legend(title='Model', bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.tight_layout()
            plt.savefig('generated/figs/time_varying_subject_cluster_barplot_combined.png', dpi=300)
            plt.close()
            print('✅ Saved: time_varying_subject_cluster_barplot_combined.png')
        else:
            print(f'⚠️ {subj_path} not found')
        # Difficulty-level plot
        diff_path = 'generated/outputs/time_varying_difficulty_level_analysis_combined.csv'
        if os.path.exists(diff_path):
            df_diff = pd.read_csv(diff_path)
            plt.figure(figsize=(12, 6))
            sns.barplot(data=df_diff, x='difficulty', y='time_to_failure_mean', hue='model', ci=None)
            plt.title('Mean Time to Failure by Difficulty Level (Time-Varying Advanced Combined)')
            plt.ylabel('Mean Time to Failure')
            plt.xlabel('Difficulty Level')
            plt.legend(title='Model', bbox_to_anchor=(1.05, 1), loc='upper left')
            plt.tight_layout()
            plt.savefig('generated/figs/time_varying_difficulty_level_barplot_combined.png', dpi=300)
            plt.close()
            print('✅ Saved: time_varying_difficulty_level_barplot_combined.png')
        else:
            print(f'⚠️ {diff_path} not found')

    def visualize_drift_cliff(self):
        """Visualize the drift cliff for each model using combined_interaction_time_varying_results.csv."""
        import pandas as pd
        import matplotlib.pyplot as plt
        import seaborn as sns
        import os
        import numpy as np
        print("\n🎨 Visualizing combined drift cliff across all models...")
        os.makedirs('generated/figs', exist_ok=True)
        df = pd.read_csv('generated/outputs/combined_interaction_time_varying_results.csv')
        drift_rows = df[df['covariate'].str.contains('cumulative_drift') & (df['Analysis_Type'] == 'Combined_Interaction_TimeVarying')]
        ...
        for i, model in enumerate(models):
            ...
            main = model_df[model_df['covariate'] == 'cumulative_drift']
            if main.empty:
                print(f"⚠️ No main drift effect for model {model}, skipping.")
                continue
            main_coef = main['coef'].values[0]
            adv_rows = model_df[model_df['covariate'].str.contains('C\(adv_id\)\[T\.') & model_df['covariate'].str.contains(':cumulative_drift')]
            adv_ids = adv_rows['covariate'].str.extract(r'C\(adv_id\)\[T\.(.*?)\]:cumulative_drift')[0].tolist()
            adv_effects = []
            adv_labels = []
        ...
        plt.ylabel('Hazard Ratio (exp(coef)) for Cumulative Drift (log scale, clipped)')
        plt.title('Combined Drift Cliff: Hazard Ratio for Cumulative Drift by Adversarial Prompt Type (All Models)\n(Values clipped to [1e-3, 1e4] for clarity)')
        plt.yscale('log')
        plt.grid(True, which='both', linestyle='--', linewidth=0.5, alpha=0.7)
        plt.legend(title='Model', bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        if any_data:
            plt.savefig('generated/figs/combined_drift_cliff_all_models.png', dpi=300)
            print("✅ Combined drift cliff plot saved: generated/figs/combined_drift_cliff_all_models.png")
        else:
            print("❌ No data available to plot combined drift cliff!")
        plt.close()

    def run_complete_analysis(self):
        """Run the complete combined time-varying advanced analysis."""
        print("🔬 TIME-VARYING ADVANCED MODELING WITH COMBINED DRIFT INTERACTIONS")
        print("=" * 60)
        print("Implementing interaction terms between adversarial prompt types and both drift types")
        print("Formula: C(adv_id) * cumulative_drift + C(adv_id) * prompt_to_prompt_drift + C(base_id) * context_to_prompt_drift + all_drift_measures\n")
        ...
        print(f"\n🎉 COMBINED TIME-VARYING ADVANCED ANALYSIS COMPLETE!")
        print("=" * 50)
        print(f"📊 Models analyzed: {len(self.models_data)}")
        print(f"🔬 Combined interaction models fitted: {len([r for r in self.interaction_results.values() if r is not None])}")
        print(f"📈 Baseline models fitted: {len([r for r in self.baseline_results.values() if r is not None])}")
        print(f"🎯 Key insight: How adversarial prompt types interact with both cumulative and immediate drift measures")

def main():
    """Run combined time-varying advanced modeling analysis."""
    analyzer = TimeVaryingAdvancedCombinedAnalyzer()
